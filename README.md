...
- два режима отображения:
  - `System` (HUD-дашборд),
  - `Classic` (список карточек).

`System`-режим вкладки `Задания` (`internal/ui/tabs/quests.go`):
- контент центрирован, ограничен по ширине (`maxWidth ~1240`);
- двухколоночный layout `65/35`:
  - слева `ACTIVE QUESTS` (журнал миссий),
  - справа `HUNTER LOG` (сводка за день);
- журнал миссий:
  - accordion по категориям `ЕЖЕДНЕВНЫЕ`, `ОСНОВНЫЕ`, `ДОЛГОСРОЧНЫЕ`,
  - `ЕЖЕДНЕВНЫЕ` и `ОСНОВНЫЕ` раскрыты по умолчанию,
  - карточки задач рендерятся через `QuestCardSystem` (`internal/ui/components/quest_card_system.go`);
- панель `HUNTER LOG` показывает:
  - `EXP` за сегодня,
  - полученные сегодня `attempts`,
  - статистику задач по категориям,
  - активную миссию (название/категория/потенциальная награда),
  - текущий streak.

Переключатель `Classic/System` вынесен из вкладки в главное меню:
- `Вид -> Theme: System HUD`
- `Вид -> Theme: Classic`
